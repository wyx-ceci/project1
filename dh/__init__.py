...
# 3072 bit safe prime for Diffie-Hellman key exchange *CHANGED FROM 1536 BIT GROUP to prevent Logjam attack*
...
def create_dh_key():
    ...
    g = 2 # generator value from MODP 4096 RFC3526
...
def calculate_dh_secret(their_public: int, my_private: int) -> bytes:
    ...
    shared_secret = str(pow(their_public, my_private, prime)).encode('utf-8') # as per rfc2631.
